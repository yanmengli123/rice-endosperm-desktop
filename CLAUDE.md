...
桌面端依赖 Yuxi 原生 Conversation + AgentRun v1.4 契约：先从 `GET /api/agent/default` 取得服务端权威智能体，再以幂等线程 ID 调用 `POST /api/chat/thread`，随后与 Web 端相同地调用 `POST /api/agent/runs`、`GET /api/agent/runs/{run_id}/events` 和 `GET /api/agent/runs/{run_id}/result`。事实型执行轨迹使用独立的 `GET /api/agent/runs/{run_id}/trace` 快照与 `GET /api/agent/runs/{run_id}/trace/events` 增量补拉；桌面端实时轮询增量，终态与重开历史会话时从最后一条本地 `assistant-{run_id}` 消息恢复权威快照，不从消息流或 UI 状态推断未发生的步骤。客户端不带 `model_spec`，由服务端按智能体策略、用户偏好和权益解析；返回的 `run_context` 必须同时匹配 protocol/agent/thread/request/result_authority，否则 fail-closed。登录三路：企业激活码 `POST /api/auth/onboarding/exchange`（一次性兑换设备会话对，绝不签发静态 Key，是 P0 合规开户路径）、设备码 `/api/auth/cli/sessions*`（兑换后须立即撤销服务端同时签发的 90 天过渡 Key——任何账号不得同时持有活会话与活静态 Key）、三要素 `/api/auth/desktop/login`（兼容模式，产出静态 Key，随版本淘汰）。会话对携带服务端签发的不可逆 `account_scope_id`（短时访问令牌 + 旋转刷新令牌 + `session_id`，本地 SQLite/Stronghold 数据按 `account_scope_id` 隔离账号，不落盘原始 uid）；会话续期走 `POST /api/auth/cli/token/refresh`（客户端按作用域单飞锁防并发重放；刷新终态失败必须清除本地会话 blob 并 fail-closed，绝不回退静态 Key）；模型偏好统一存服务端 `GET/PUT /api/user/model-preference`。完整清单见 README「使用前提」。修改这些接口时，服务端（../Yuxi）与本仓库必须同步调整。
